...
NOTA sobre tcn.py:
    Este pipeline NO usa el main() de tcn: importa solo sus piezas reutilizables
    (modelos, helpers de entrenamiento y de inferencia). En particular:
      - run_inference se IMPORTA de tcn (su versión ya acepta 'coverage' y genera
        la columna 'r_pred', así que no hace falta duplicarla aquí).
      - apply_trend_gate se mantiene LOCAL a propósito: tcn exporta otra función
        con el mismo nombre pero comportamiento distinto (gate basado en error de
        reconstrucción del CNN-AE, que exige un parámetro 'recon_anomaly' que este
        pipeline no calcula). Importarla cambiaría la estrategia, así que se
        conserva aquí la versión bidireccional tendencia + drawdown.
"""
...
from tcn import (
    ...
    run_inference,                       # se reutiliza la inferencia de tcn (no se duplica)
)
from metrics import (
...
# Instancia ligera reutilizada SOLO para DataProcessor.create_sequences (ventaneo).
# Evita reimplementar la lógica de ventanas en este archivo (única fuente: load_data).
_DP = DataProcessor()
...
def make_windows(d, ws=WINDOW_SIZE):
    ...
    Delega en DataProcessor.create_sequences para que el ventaneo tenga UNA sola
    implementación (la de load_data) y no pueda divergir de la del resto del
    proyecto. Se reutiliza una instancia ligera (_DP) porque create_sequences no
    depende de su estado interno.
    """
    return _DP.create_sequences(d, window_size=ws)
...
# ══════════════════════════════════════════════════════════════════════════════
# INFERENCIA DEL SEMÁFORO
# ══════════════════════════════════════════════════════════════════════════════
# run_inference se IMPORTA de tcn (arriba): su versión ya asigna por rango la
# cobertura deseada y devuelve la columna 'r_pred', de modo que no se duplica.
#
# apply_trend_gate, en cambio, se mantiene LOCAL a propósito. tcn exporta otra
# función llamada igual pero con comportamiento DISTINTO (gate por error de
# reconstrucción del CNN-AE, que requiere un 'recon_anomaly' que este pipeline no
# construye). Usar la de tcn cambiaría la estrategia y, sin ese array, dejaría
# pasar las señales casi sin filtrar. Por eso aquí se conserva la versión
# bidireccional (tendencia SMA + override de drawdown).
