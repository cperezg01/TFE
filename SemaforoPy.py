...
def main():
    ...
    signals = apply_trend_gate(run_inference(agent, z_test, test_dec_dates, DEVICE, coverage=COVERAGE), df)
...
def run_inference(agent, z_sequences, dates, device, q_low=0.33, q_high=0.67):
    """
    Decisión por TERCILES de la señal alcista s = P(BUY) - P(SELL):
      s alto  → BUY,  s bajo → SELL,  medio → HOLD.
    Garantiza las tres clases aunque la salida del modelo sea casi constante.
    """
    agent.eval()
    P = []
